...
<div>
    <img width="330" alt="image" margin='auto' src="https://github.com/HyenC/MealKit/assets/38906574/dd64f7ce-631f-428b-8bad-b0346f789b43">
    <img width="330" alt="image" src="https://github.com/HyenC/MealKit/assets/38906574/0e9a512c-0e83-4fad-b69c-aab49f05948f">
    <img width="330" alt="image" src="https://github.com/HyenC/MealKit/assets/38906574/3f7b74ef-fc2a-457f-b749-ca35a30aee24">
</div>
...
<div>
    <img width="500" alt="image" margin='auto' src="https://github.com/HyenC/MealKit/assets/38906574/3f132c19-8726-4643-9c6a-e16d2abc3baa">
    <img width="500" alt="image" margin='auto' src="https://github.com/HyenC/MealKit/assets/38906574/56321efc-5554-4b66-9a45-92cdc972c6d0">
</div>
...
<div>
    <img width="500" alt="image" src="https://github.com/HyenC/MealKit/assets/38906574/1dd35942-b35e-4a1e-a2d7-c39e308f6617">
    <img width="500" alt="image" src="https://github.com/HyenC/MealKit/assets/38906574/ccecce4d-c2e2-4b20-b5ff-0ffcb5e13cf4">
</div>
...
<div>
    <img width="500" alt="image" src="https://github.com/HyenC/MealKit/assets/38906574/f3083150-94c7-4eaf-a861-bf994bc40e5f">
    <img width="500" alt="image" src="https://github.com/HyenC/MealKit/assets/38906574/bc16ebd6-4a49-4807-bae6-15e9c4652b64">
</div>
...
<div>
    <img width="500" alt="image" src="https://github.com/HyenC/MealKit/assets/38906574/325e46a8-3597-4d51-ad53-dfa98b7f4f61">
    <img width="500" alt="image" src="https://github.com/HyenC/MealKit/assets/38906574/94b0e9a6-baa9-4196-95e3-48f767855a13">
</div>
...
<img width="688" alt="image" src="https://github.com/HyenC/MealKit/assets/38906574/c8825d02-3f23-4dec-b48b-057e6944158c">
...
<img width="688" alt="image" src="https://github.com/HyenC/MealKit/assets/38906574/a1f62e6a-e05d-444b-bc7b-ab43f9344cf9">
...
<img width="688" alt="image" src="https://github.com/HyenC/MealKit/assets/38906574/66c7c011-4e25-414c-9bf2-601ef4933cb7">
...
<div>
    <img width="500" alt="image" src="https://github.com/HyenC/MealKit/assets/38906574/151becf7-1f41-46e5-86cc-d5dfeef6628b">
    <img width="500" alt="image" src="https://github.com/HyenC/MealKit/assets/38906574/ff3dd0e8-b38e-4c9d-92bf-5ffd80cb7840">
</div>
...
#### 첫째, 스테디셀러인 '면류' 밀키트를 활성화 시켜야 한다. </summary>
<div markdown="1">

- 앞서 분석한 ‘2018~2020년 간편식 구입변화(%) 데이터’를 통해 밀키트 품목 중 ‘면류'는 꾸준히 각광 받고 있다는 것을 알 수 있었다. 이러한 점을 이용하여 경쟁력 있는 ‘면류' 밀키트를 만들어 밀키트 사업의 수요를 더 증가시킬 방법을 모색해보았다. 우선 전국의 면 음식을 주 메뉴로 하는 맛집 리스트를 분석하고 밀키트와 연관지어 생각해보았다. 현재 우리 사회는 유통물류 관련해서 큰 발전을 이루었는데, 이때 빠질 수 없는 것이 전국을 연결하는 배달 및 운송 시장의 성장이다. 운송시장의 성장으로 직접 찾아가지 않더라도 빠른 시일내에 식품 배달을 받을 수 있게 되었는데, 이를 밀키트에 접목시켜 누구나 간편하게 전국 맛집의 음식을 섭취할 수 있도록 해야 한다. 예를 들어 막국수 같은 경우, 춘천의 막국수를 모티브로 밀키트를 만들어 해당 지역의 느낌을 살린 식품을 전국 어디에서든 먹을 수 있도록 하는 것이다. 이는 위의 ‘연도별 식품소비 트렌드별 동조성 데이터’를 분석한 내용으로도 뒷받침 할 수 있다. ‘연도별 식품소비 트렌드 데이터’에서는 19년도에서 21년도로 갈수록 ‘새롭고 다양한 맛'을 원하는 소비자들이 증가하고 있다는 것을 확인할 수 있는데, 전국에서 유명한 맛집들의 주 메뉴를 밀키트로 만들어 판매한다면 소비자들의 식품소비 트렌드를 따라가면서도 그들의 니즈를 충족 시킬 수 있을 것이다.
...
#### 둘째, 밀키트에 유통기한이나 영양성분들이 제대로 표기되어 있지 않은 제품이 많은 문제를 해결해야 한다. </summary>
<div markdown="1">

- 실제로 ‘간편식을 구입하지 않는 이유’ 데이터를 분석해본 결과 주로 밀키트에 안정성(신선도, 유통기한)이 염려되거나 원재료와 원산지의 품질의 의심되어서 구입이 꺼려진다는 의견이 많다는 것을 알 수 있다. 간편식에 대한 이러한 부정적인 이미지가 소비자들에게 각인되면 향후 간편식 산업에도 큰 영향을 끼칠 것이다. 간편식 시장을 지속적으로 성장시키기 위해서는 정부에서 밀키트 유통기한과 영양성분들을 의무적으로 표기하도록 법을 신속히 집행하는 등의 노력이 필요하고, 무인 밀키트 프렌차이즈에서는 매장 관리에 각별한 주의가 필요할 것이다.
...
#### 셋째, 1인 가구, 노인층을 겨냥하려면 가격 경쟁력을 높여야 한다. </summary>
<div markdown="1">

- 1인 가구의 경우 독거노인이나 학생 및 사회초년생이 대다수이고, 이들은 모두 경제적으로 여유롭지 않다는 특징이 있다. 따라서 1인가구 소비자들을 타켓팅하기 위해서는 가격 경쟁력을 높여야 한다. 가격 경쟁력을 높이기 위해서는 공동구매 방식을 사용하는 것이 효율적인데, 이는 같은 소비성향을 가진 사람들을 모을 수 있는 커뮤니티 성격을 가진 플랫폼이 필요하다. 이러한 플랫폼을 구축하고, 그 안에서 같은 상품을 구매하기를 원하는 사람들을 그룹화하여 공동구매함으로써 가격을 낮출 수 있고, 판매하는 업체의 입장에서도 많은 양의 상품을 판매하니 밀키트 시장이 성장하는데에 큰 도움이 될 것이다.
...
#### 넷째, 노인들의 특성이나 여러가지 요인들을 고려한 밀키트를 제공해야 한다. </summary>
<div markdown="1">

- 노인층 소비자들은 요리를 해먹기가 힘든 경우가 많고, 특히 독거노인이라면 매 끼니를 해결하는 데 경제적으로나 신체적으로 힘든 부분이 있다. 그래서 노인층들이 밀키트를 대량으로 비축해두고 끼니를 해결한다면 간편하게 조리를 할 수 있고 경제적 부담이 덜 수 있는데, 이를 실현시키려면 노인복지센터와 손을 잡고 협업을 하는 방식을 선택해야 한다. 직접 매장에 가서 밀키트를 구입하기 힘든 부분이 있을 수 있고 온라인이나 어플을 통한 구매에도 익숙치 않기 때문에 어플을 이용하여 구매하는데도 제한적이다. 따라서 노인복지센터에 노인층들을 위한 밀키트를 판매하고 센터의 복지사들이 지급하는 방식으로 노인층을 타겟팅 해야 할 것이다. 또 노인들을 위한 간편식들이 출시되긴 하지만 노인이라고 하여 ‘건강'에만 초점을 맞추고 ‘맛’은 고려하지 않는 경우가 많다. 따라서 노인은 무조건 건강한 식단만을 선호할 것이라는 편견을 버리고 다양한 맛을 선택할 수 있도록 해야 하고 맛과 영양의 균형을 추구해야 한다. 오히려 가끔 간편식을 먹는 것이 아닌 주식으로 간편식을 먹거나 간편식 섭취 비중이 높을수록 다양한 식단 구성에 신경써야 한다.
...
#### 다섯째, 원료 안전성을 보장해야 한다. </summary>
<div markdown="1">

- 국내 밀키트 시장의 대부분의 상품은 냉장 밀키트이다. 이와 같은 냉장 밀키트는 4~5일 정도 되는 짧은 유통기한을 가지고 있어, 소비자 입장에서는 유통기한을 신경써서 상품을 구매해야하고, 판매자 입장에서는 짧은 유통기한 동안 상품이 팔리지 않는 경우 폐기처분 해야하는 단점이 있다. 이러한 단점을 보완하기 위해 냉동 밀키트 상품을 만들고 상품의 유통기한을 늘려 소비자에게 상품의 안전성에 대한 믿음을 주고, 판매자에게는 재고 관리를 더 편리하게 할 수 있도록 해야 한다.  최근 외식업계에서 푸드테크의 도입이 활발히 이루어지고 있다. 매장에 서빙로봇을 배치하는 푸드테크를 도입하며 인건비를 줄이는 전략을 짜는 등 외식업계의 트렌드가 변화 중이다. 이러한 관점에서 밀키트에 도입할 수 있는 기술을 고민하던 중 ‘무인 판매가 가능한 스마트 자판기’를 활용하는 사례를 마주했다. 소비자가 자판기 냉장고에 카드를 꽂고 상품을 꺼내가면 자동으로 결제되는 시스템으로 자판기를 운영하는 업주는 스마트폰으로 실시간 재고 관리와 내부 온도 체크, 고장 여부 등을 확인할 수 있어 기존 밀키트 무인 매장의 단점이었던 재고 관리를 효율적으로 할 수 있고 상품에 대한 안정성을 높일 수 있다. 또 외식업계에서 서빙로봇을 배치하는 것과 같이 초기 비용을 지불하면 인건비를 줄일 수 있는 효과가 있고, 매장을 무인으로  24시간 운용할 수 있다. 데이터 분석 과정에서 알 수 있듯이 간편식을 구입하지 않는 이유와 간편식에 대한 인식이 상품의 안전성이 떨어진다는 의견이 많았는데, 이와 같은 푸드테크를 도입하고 상품에 대한 안정성과 신뢰도를 높이면 사람들의 밀키트에 대한 인식이 긍정적으로 바뀌고 많은 소비자가 무인 밀키트 매장을 찾는 효과를 얻을 수 있을 것이다.
...
#### 여섯째, 유통 트렌드에 맞추어 밀키트 사업에서도 차별화를 할 수 있는 전략을  출시해야 한다. </summary>
<div markdown="1">

- 최근 2030 사이에서 오마카세가 인기를 끌고 있는데 그중에서도 ‘파스타 오마카세’를 찾는 사람들이 늘고 있다. 특히 앞선 분석에서 ‘면류'가 간편식을 찾는 사람들에게 꾸준히 인기라는 것을 고려했을 때 한 키트 안에 다양한 종류의 파스타를 넣어 다양화한다면 골라먹는 재미와 자신의 독특한 취향을 찾는 데 도움이 될 것이다. 예를 들어 파스타 면에는 숏 파스타, 생면, 링귀니 등이 있는데 이를 조금씩 모두 넣는다면 집에서도 오마카세를 먹는 듯한 느낌을 줄 수 있을 것이다. 또한 동아일보(2021)에 따르면 최근 급부상한 건강식, 채식 수요를 바탕으로 사찰음식에 기반한 간편식 제품이 출시되었는데 그 매출이 전년 동기 대비 6기 가량 급증했다. 휴식을 위해 ‘템플스테이’를 선택하는 청년들이 늘어나는 트렌드를 파악한 것도 한 몫을 했다. 이처럼 최신 유행에 민감한 관심을 기울이고 개별 특색을 살릴 수 있는 새로운 차별화 전략을 앞세운다면 성공을 거둘 수 있을 것이다.
...
#### 일곱째, 간편식도 직접 조리하는 음식에 준하는 정성스런 음식이라는 인식을 제고할 수 있는 방안을 모색해야 한다. </summary>
<div markdown="1">

‘간편식을 구입하지 않는 이유’에 대한 설문 데이터를 분석한 결과 가족의 식사는 직접 요리해야한다고 생각하는 사람들이 꽤 많은 비중을 차지하는 것을 볼 수 있었다. 전술했듯이 밀키트의 경우 HMR과 달리 식재료 그 자체인 경우가 많은데, 많은 사람들이 그 차이점에 대해 인식하지 못한다. 또한 겉포장지만 보고서는 밀키트와 HMR을 구별할 수 없는 경우가 부지기수이다. 분석한 데이터 결과에서 도출한 한국인의 정서를 고려한다면 밀키트를 사용하여 가족 식사를 대접하는 경우 음식을 만드는 단계에서 가족들의 눈치를 보게 되는 상황이 발생할 수 있다. 이때 겉포장지에 완제품의 일러스트를 넣지 않고 투명하게 하여 식재료가 잘 보이게 한다면 가족들도 밀키트가 가공식품이라는 생각이 들지 않을 수 있고 요리하는 사람도 마음 편히 요리를 할 수 있을 것이다. 상품 이미지에 대한 설명이 필요한 경우 오프라인 매장은 상품 진열대에, 온라인은 상세 페이지에 적거나 밀키트의 뒷면에 이미지를 첨부할 수 있을 것이다.
